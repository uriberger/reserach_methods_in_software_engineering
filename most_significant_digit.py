...
def long_most_significant_digit(
    my_list, large_num, small_num, include_factor, exclude_factor
):
    res = ""
    first = True
    for cur_num in my_list:
        if cur_num < large_num:  # Multiple if statements
            if cur_num > small_num:
                include_factor_remainder = (
                    cur_num % include_factor
                )  # Temporary variable
                if include_factor_remainder == 0:
                    exclude_factor_remainder = (
                        cur_num % exclude_factor
                    )  # Temporary variable
                    if exclude_factor_remainder != 0:
                        ...
                        most_significant_digit_str = str(
                            most_significant_digit_int
                        )  # Temporary variable
                        if first:
                            first = False
                        else:
...
def short_most_significant_digit(
    my_list, large_num, small_num, include_factor, exclude_factor
):
   res = []
   for cur_num in my_list:
    if (
        large_num > cur_num > small_num  # Comparison chain
        and cur_num % include_factor == 0
        and cur_num % exclude_factor != 0
        ):  # Single line computation, complex boolean expression
        ten_power = 1
        while cur_num > ten_power:
            ten_power *= 10  # Use of a compound assignment operator
        ten_power /= 10  # Use of a compound assignment operator
        res += [math.floor(cur_num / ten_power)]
    return ",".join(res)
...
print(
    long_most_significant_digit(
        my_list, large_num, small_num, include_factor, exclude_factor
    )
)
print(
    short_most_significant_digit(
        my_list, large_num, small_num, include_factor, exclude_factor
    )
)
