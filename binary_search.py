...
Tested features:
- Use of temporary variables vs. a complex computation
'''
...
def long_binary_search(ordered_list, term):
    ...
    while high_border - low_border > 1:
        ...
        half_len = int(cur_len/2) # Temporary variable
...
def short_binary_search(ordered_list, term):
    ...
    high_border = len(ordered_list)
    while high_border - low_border > 1:
        if term == ordered_list[low_border+int((high_border-low_border)/2)]: # No temporary variable
            return True
        elif term < ordered_list[low_border+int((high_border-low_border)/2)]: # No temporary variable
            high_border = low_border+int((high_border-low_border)/2) # No temporary variable
        else:
            low_border = low_border+int((high_border-low_border)/2) # No temporary variable
    return False
